...
export function PasswordModal({ caseSlug, onClose, labels }: PasswordModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setPending(true)
    try {
      const result = await validateCasePassword(caseSlug!, password)
      if (result.success) {
        onClose()
        router.push(`/${locale}/cases/${caseSlug}`)
      } else {
        setError(errorMsg)
      }
    } catch {
  ...
  return (
    <div className="password-modal" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className="password-modal-inner">
        ...
        <ModalCloseFab onClick={onClose} label="Close" />
      </div>
    </div>
  )
}
